...
new_url = st.sidebar.text_input(
    "Add new URL", placeholder="https://www.lazyportfolioetf.com/..."
)
...
for i, url in enumerate(st.session_state.urls):
    ...
    display_name = url.split("/")[-2] if url.endswith("/") else url.split("/")[-1]
    col1.text(f"{i+1}. {display_name}")
...
if st.sidebar.button("Fetch Data"):
    with st.spinner("Fetching data from lazyportfolioetf.com..."):
        ...
        if not df.empty:
            ...
            st.success(
                f"Successfully fetched data for {len(df)} assets! ({len(errors)} errors)"
            )
            ...
            status_container.error(
                "No data could be scraped. Check the error details below."
            )
            ...
            if errors:
                error_summary = "\n".join(
                    [f"• {e['url']}: {e['error']}" for e in errors[:3]]
                )
                st.error(f"First few errors:\n{error_summary}")
                if len(errors) > 3:
                    st.error(
                        f"...and {len(errors) - 3} more errors. See the Errors tab for details."
                    )
            else:
                st.error(
                    "No specific errors were reported, but no data was fetched. Check URL format and website structure."
                )
...
st.sidebar.caption(
    "Data Source: [lazyportfolioetf.com](https://www.lazyportfolioetf.com)"
)
...
if st.session_state.data is not None and not filtered_df.empty:
    ...
    col1.metric(
        ...
        len(filtered_df),
        delta=(
            f"{len(filtered_df) - len(st.session_state.data)}"
            if len(filtered_df) != len(st.session_state.data)
            else None
        ),
    )

    col2.metric(
        "Avg Nominal Return", f"{filtered_df['nominal_30y_return'].mean():.2f}%"
    )

    col3.metric("Avg Real Return", f"{filtered_df['real_30y_return'].mean():.2f}%")

    col4.metric("Avg Std Deviation", f"{filtered_df['std_deviation'].mean():.2f}%")
    ...
    with tab1:
        ...
        st.info(
            "📊 **Table Highlights:** Green = Best Nominal Sharpe, Blue = Best Real Sharpe"
        )

        # Add clickable links to original sites
        st.markdown("### Links to Original Sites")
    ...
    with tab4:
        ...
        if selected_assets:
            ...
            st.dataframe(
                format_dataframe_for_display(comparison_df), use_container_width=True
            )
        else:
            ...
            for i, error in enumerate(st.session_state.errors):
                with st.expander(f"Error {i+1}: {error['url'].split('/')[-2]}"):
                    ...
                    if "Missing metrics" in error["error"]:
                        ...
                        st.markdown(
                            "- Content might be loading dynamically with JavaScript"
                        )
                        st.markdown(
                            "- Required metrics might use different naming patterns"
                        )
    ...
    st.warning(
        "No assets match the current filter criteria. Try adjusting the filters."
    )
else:
    ...
    st.info(
        "👆 Click 'Fetch Data' to see the interactive dashboard with real metrics and visualizations."
    )
